...
def generate_random_list(N, Xmin, Xmax, dX, Ymin, Ymax, dY, max_step, Nrepmin, Nrepmax, dNrep, n_extra=0, touch_limits=False):
    np.random.seed(9)

    min_x_glob = np.round(Xmin - n_extra*dX, 6)
    max_x_glob = np.round(Xmax + n_extra*dX, 6)
    min_y_glob = np.round(Ymin - n_extra*dY, 6)
    max_y_glob = np.round(Ymax + n_extra*dY, 6)
    print(min_x_glob, max_x_glob, min_y_glob, max_y_glob)
    niter = 0
    while True:
        x_list = [[1] * Nrepmin]
        y_list = [[1] * Nrepmin]
        size = Nrepmin
        while size<N:
            which_change = np.random.randint(0, 2)
            Nrep = np.random.choice(np.arange(Nrepmin, Nrepmax+dNrep/2, dNrep, dtype=int), size=1)[0]
            prev_x = x_list[-1][-1]
            prev_y = y_list[-1][-1]
            if which_change==0:
                min_x = max(prev_x - max_step*dX, min_x_glob)
                max_x = min(prev_x + max_step*dX, max_x_glob)
                x_list.append([np.random.choice(np.arange(min_x, max_x + dX/2, dX), size=1).round(6)[0]] * Nrep)
                y_list.append([prev_y] * Nrep)
            else:
                min_y = max(prev_y - max_step*dY, min_y_glob)
                max_y = min(prev_y + max_step*dY, max_y_glob)
                y_list.append([np.random.choice(np.arange(min_y, max_y + dY/2, dY), size=1).round(6)[0]] * Nrep)
                x_list.append([prev_x] * Nrep)
            size += Nrep
        
        x_list = np.concatenate(x_list)[:N]
        y_list = np.concatenate(y_list)[:N]
        if not touch_limits or (min_x_glob in x_list and max_x_glob in x_list and min_y_glob in y_list and max_y_glob in y_list):
            break
        niter += 1 
    print(niter)
...
#%% Depletion steps
Nsteps = 1500
power_frac, threshold_frac = generate_random_list(N=Nsteps, Xmin=0.6, Xmax=1, dX=0.1, Ymin=0.9, Ymax=1.1, dY=0.05, max_step=2, Nrepmin=50, Nrepmax=100, dNrep=10, n_extra=1, touch_limits=True)
power_normalization_field = 'power'

power_normalization_value = 10e6 * power_frac # W
neutrons_per_cycle = 200000  #*20 + [10000]*20 + [10000]*(Nsteps-20-20-50) + [50000]*50
decay_step = 0 # days

#%% Burnup cycle
threshold_type = 'burnup'
threshold = 72 * threshold_frac
...
#%% Outputing
output_folder_name = 'HTR-10_transient_challenging_restart' # Name of the output folder
verbosity_mode = 1
